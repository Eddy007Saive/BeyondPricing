...
export function Calendrier() {
  ...
  // Chargement des données de scoring en fonction du logement sélectionné
  useEffect(() => {
    const loadScoringData = async () => {
      ...
      try {
        ...
        // Utilisation de la fonction optimisée qui filtre directement côté serveur
  ...
  const getDataForDate = (date) => {
    ...
    const result = scoringData.find(d => d.date === dateStr);
  ...
  // Obtenir les informations du logement sélectionné
  const getSelectedLogementInfo = () => {
    return logements.find(logement =>
      logement.idBeds24 === selectedLogement ||
      logement.id === selectedLogement
    );
  };
...
`}
      ...
      >
        ...
        {data && (
          <div className="space-y-1">
            ...
            {/* Météo */}
            {data.meteo && data.meteo !== 'Non disponible' && (
        ...
        {/* Debug indicator - montre s'il y a des données */}
        {data && (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="text-xs text-gray-600 mt-1">
              Taux: {Number(data.tauxOccupation)*100}%
            </div>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            {selectedLogementInfo && (
              <div className="mt-2 opacity-75 text-sm">
                {selectedLogementInfo.nom} - {selectedLogementInfo.ville} ({selectedLogementInfo.typologie})
              </div>
            )}
          </div>
          <div className="text-right">
            ...
            <select
              ...
              className="bg-white/20 border border-white/30 rounded-lg px-3 py-2 text-white placeholder-white/70 backdrop-blur-sm min-w-[200px]"
            >
              <option value="" className="text-gray-900">Sélectionner un logement</option>
              {logements.map(logement => (
                <option key={logement.id} value={logement.idBeds24 || logement.id} className="text-gray-900">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex bg-white/20 rounded-lg p-1">
              <button
                ...
                className={`px-3 py-1 rounded text-sm font-medium transition-colors ${viewMode === 'month' ? 'bg-white/30' : 'hover:bg-white/20'
                  }`}
              >
                Mois
              </button>
              <button
                ...
                className={`px-3 py-1 rounded text-sm font-medium transition-colors ${viewMode === 'week' ? 'bg-white/30' : 'hover:bg-white/20'
                  }`}
              >
                Semaine
              </button>
      ...
      {/* Message si aucune donnée de scoring */}
      {selectedLogement && scoringData.length === 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-center gap-2 text-yellow-800">
            <Info className="h-5 w-5" />
            <span className="font-medium">Aucune donnée de tarification trouvée pour ce logement</span>
          </div>
          <p className="text-yellow-700 text-sm mt-1">
            Vérifiez que des données de scoring existent pour ce logement dans Airtable.
          </p>
        </div>
      )}
      ...
      {/* Informations du logement sélectionné */}
      {selectedLogementInfo && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <h3 className="text-lg font-semibold text-gray-900 mb-3">Informations du logement</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="font-medium">{selectedLogementInfo.capacite} personnes</div>
            </div>
            <div>

...
import {
  ...
  // Mock du service scoring - remplacez par votre vrai service
  const mockScoringService = {
  getScorings: async (params = {}) => {
  ...
  prixMarche: 120 + Math.random() * 80,
  tension: ['Faible', 'Modérée', 'Élevée'][Math.floor(Math.random() * 3)],
  meteo: ['Ensoleillé', 'Nuageux', 'Pluvieux', 'Neigeux'][Math.floor(Math.random() * 4)],
  tauxOccupation: (60 + Math.random() * 40).toFixed(1) + '%',
  evenement: Math.random() > 0.8 ? 'Festival local' : '',
  tarifIA: 110 + Math.random() * 90,
  promoIA: Math.random() > 0.9 ? '10%' : '',
  idLogement: 'LOG001',
  minPrice: 80,
  justification: 'Optimisation basée sur la demande du marché',
  weekEnd: [0, 6].includes(date.getDay()) ? 'Oui' : 'Non',
  confiance: (80 + Math.random() * 20).toFixed(0) + '%',
  strategie: ['Conservative', 'Agressive', 'Équilibrée'][Math.floor(Math.random() * 3)],
  });
  }
  
  return {
  data: {
  scorings: scorings.filter(s => {
  if (params.search) {
  return s.idLogement.toLowerCase().includes(params.search.toLowerCase()) ||
         s.strategie.toLowerCase().includes(params.search.toLowerCase());
  }
  return true;
  }),
  totalItems: scorings.length,
  totalPages: 1,
  currentPage: 1
  }
  };
  ...
  const [selectedLogement, setSelectedLogement] = useState('LOG001');
  ...
  try {
  setLoading(true);
  const response = await mockScoringService.getScorings({
  search: selectedLogement
  });
  ...
  >
    <option value="LOG001">LOG001 - Appartement Centre</option>
    <option value="LOG002">LOG002 - Villa Bord de Mer</option>
    <option value="LOG003">LOG003 - Studio Moderne</option>
  </select>
  </div>
  </div>
  ...
  <div>
    ...
    <div className="text-xl font-bold text-gray-900">
      {Math.round(scoringData.reduce((acc, d) => acc + d.tarifIA, 0) / scoringData.length || 0)}€
    </div>
  </div>
  </div>
  </div>
  ...
  <div>
    ...
    <div className="text-xl font-bold text-gray-900">
      {scoringData.filter(d => d.tarifIA > d.prixMarche * 1.05).length}
    </div>
  </div>
  </div>
  </div>
  ...
  <div>
    ...
    <div className="text-xl font-bold text-gray-900">
      {Math.round(scoringData.reduce((acc, d) => acc + parseFloat(d.confiance), 0) / scoringData.length || 0)}%
    </div>
  </div>
  </div>
  </div>
